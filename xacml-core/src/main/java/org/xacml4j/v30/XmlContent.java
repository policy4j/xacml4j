...
public final class XmlContent implements Content
{
    ...
    private Optional<BagOfAttributeValues> getAttributeValues(
            ...
            throws PathEvaluationException
    {
        ...
        {
            if(callback != null) {
                Collection<AttributeValue> v = callback.get().getAttributeValues(
                        (selectorKey.getContextSelectorId() == null ? CONTENT_SELECTOR : selectorKey.getContextSelectorId()),
                        XacmlTypes.XPATH);
                if (v.size() > 1) {
                    throw new PathEvaluationException(Status.syntaxError().build(),
            ...
            throw PathEvaluationException.wrap(e);
        }
    }
     ...
     */
    private Optional<BagOfAttributeValues> toBag(String xpath,
                                                 AttributeValueType type, NodeList nodeSet)
            throws PathEvaluationException
    {
        BagOfAttributeValues.Builder b = type.bag();
     ...
     */
    private AttributeValue convertNodeToXacml(
            String xpath, Node n, AttributeValueType type)
    {
        String v;
        ...
        AttributeValue value = toString.get().fromString(v);
        if (LOG.isDebugEnabled()) {
