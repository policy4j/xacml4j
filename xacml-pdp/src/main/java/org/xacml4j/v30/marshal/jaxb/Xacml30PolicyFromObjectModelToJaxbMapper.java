...
class Xacml30PolicyFromObjectModelToJaxbMapper
{
	...
	public JAXBElement<?> toJaxb(DecisionRule d){
		...
		if(d instanceof PolicyReference){
			return toJaxb((PolicyReference)d);
		}
		if(d instanceof PolicySetReference){
			return toJaxb((PolicySetReference)d);
		}
	...
	private JAXBElement<?> toJaxb(PolicyReference ref)
	{
	...
	private JAXBElement<?> toJaxb(PolicySetReference ref)
	{
		IdReferenceType jaxbRef = factory.createIdReferenceType();
		if(ref.getEarliestVersion() != null){
			jaxbRef.setEarliestVersion(ref.getEarliestVersion().getPattern());
