...
class AnnotatedResolverFactory
{
	...
	static{
		ATTR_RESOLVER_RETURN_TYPE = new TypeToken<Map<String, BagOfValues>>(){};
	}
		...
		throws SyntaxException
	{
		...
		final Invocation<Map<String, BagOfValues>> invocation = new Invocation(instance, m, info.first());
		...
		throws SyntaxException
	{
		...
		for(Annotation[] p : m.getParameterAnnotations())
		{
			...
			{
				if(!(types[i].equals(BagOfValues.class))){
					throw SyntaxException.invalidResolverMethod(m,
							String.format("Resolver method parameter at index=\"%d\" must be of type=\"%s\"",
							i, BagOfValues.class.getName()));
				}
				XacmlAttributeDesignator ref = (XacmlAttributeDesignator)p[0];
				java.util.Optional<ValueType> type = XacmlTypes.getType(ref.dataType());
					...
					p[0] instanceof XacmlAttributeSelector)
			{
				if(!(types[i].equals(BagOfValues.class))){
					throw SyntaxException.invalidResolverMethod(m,
							...
							m.getName(), i, BagOfValues.class.getName()));
				}
	...
	public static final class Invocation <T> implements Function<ResolverContext, T>
	{
		...
		public T invoke(ResolverContext context)
		{
			try
			{
				Map<AttributeReferenceKey, BagOfValues> keys = context.getResolvedKeys();
				if (requiresContext) {
