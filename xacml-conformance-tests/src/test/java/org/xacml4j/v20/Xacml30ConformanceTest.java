...
public class Xacml30ConformanceTest
{
	/** Log */
	private static final Logger LOG = LoggerFactory.getLogger(Xacml30ConformanceTest.class);
	...
	public static void init_static() throws Exception {
		repository = new InMemoryPolicyRepository(
				"Xacml30ConformanceTestRepository",
		...
		responseMarshaller = MarshallingProvider.getProvider(MediaType.Type.XACML30_XML)
	...
	@Before
	public void init(){
		this.pdpBuilder = PolicyDecisionPointBuilder.builder("Xacml30ConformanceTestPdp")
		.pip(PolicyInformationPoint.builder("Xacml30ConformanceTestPip")
	...
	private void executeTestCase(String testPrefix, int testCaseNum) throws Exception
	{
		...
		Xacml30TestUtility.assertResponse(getResponse(testPrefix, testCaseNum), actual);
	}

	private static Supplier<InputStream> getPolicy(
			String prefix, int number, String suffix) {
		String path = "xacml3.0-ct-v.0.4/" + createTestAssetName(prefix, number, suffix);
	...
	private static void addPolicy(PolicyRepository r, String prefix, String suffix, int index)
	{
		try{
			...
			CompositeDecisionRule rule = tool.importPolicy(MediaType.Type.XACML30_XML, policyStream);
			if (rule == null) {
	...
	private static ResponseType getResponse(String prefix, int num) throws Exception {
		return Xacml30TestUtility.getResponse("xacml3.0-ct-v.0.4/" + createTestAssetName(prefix, num, "Response.xml"));
	}

	private static RequestContext getRequest(String prefix, int number) throws Exception {
		return Xacml30TestUtility.getRequest("xacml3.0-ct-v.0.4/" + createTestAssetName(prefix, number, "Request.xml"));
	}
