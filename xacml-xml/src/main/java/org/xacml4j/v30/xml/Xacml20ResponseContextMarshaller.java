...
public final class Xacml20ResponseContextMarshaller
	...
	static class Mapper
	{
		...
		private ObligationType create(Advice advice)
		{
			...
			obligation.getAttributeAssignment().addAll(to(advice.getAttributes()));
			return obligation;
		}
		...
		private ObligationType create(Obligation o)
		{
			...
			obligation.getAttributeAssignment().addAll(to(o.getAttributes()));
			return obligation;
		}

		public Collection<AttributeAssignmentType> to(Collection<AttributeAssignment> v){
			return v.stream().map(a->create(a))
